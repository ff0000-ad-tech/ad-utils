...
/**
	...
	@param {object} parent
		parent object to which target keys are re-assigned.
	@desc
	...
	@example
		...
		ObjectUtils.assignValueAtKey(alignObj, 'against', View.main)
		// alignObj.x.against === View.main['logo'] = View.main.logo
*/
export function assignValueAtKey(obj, targetKey, parent) {
	parent = parent || {}
	for (var subKey in obj) {
		// reassign the target key from a string to a child of the parent object if a parent is defined
		if (subKey === targetKey && typeof obj[subKey] === 'string') obj[subKey] = parent[obj[subKey]]
		// if the value at the subKey is also an object, continue digging
		if (obj[subKey] && obj[subKey].toString() === '[object Object]') assignValueAtKey(obj[subKey], targetKey, parent)
	}
}

 ...
 * };
 *
 * let result = ObjectUtils.assign( customObj, defaultObj, true );
 *
 ...
 *
 * //if not recursive
 * let result = ObjectUtils.assign( customObj, defaultObj, false );
 *
 ...
 * }
 */
function assign(obj, defaultObj, recursive) {
	obj = obj || {}
	...
	for (let key in defaultObj) {
		...
		if (result[key] === undefined) {
			...
			if (Object.prototype.toString.call(item) === '[object Object]') {
				result[key] = assign(result[key], item, recursive)
			}
		}
	}
...
export { defaults, assign, objectifier }

/**
 * @memberOf ObjectUtils
 * @method assignValueAtKey
 * @param {object} parent
 * 	parent object to which target keys are re-assigned. If undefined, will not make any changes.
 * @param {string} targetKey
 * 	string for which key within the obj to target
 * @param {object} target
 * 	object to dig through
 * @desc
 * 	Dig through an object and convert a specific key to a parent object's key element. Useful for when references to elements need to be made before that element exists. Will not return a new object, but instead changes the given object.
 * @example
 * // Write an alignment object before any UIComponents have been created
 * let alignObj = {
 * 	x:{
 * 		type: Align.LEFT,
 * 		against: 'logo'
 * 	}
 * };
 * // alignObj.x.against === 'logo'
 *
 * // Create some UIComponents
 * View.main.logo = new UIImage(...)
 *
 * // Reassign the alignment object's "against" value to target a child of View.main
 * ObjectUtils.assignValueAtKey(View.main, 'against', alignObj)
 * // alignObj.x.against === View.main['logo'] = View.main.logo
 *
 * // Make a new UIImage that will use the alignment object to align itself against View.main.logo
 * const _newImage = new UIImage(...)
 * Align.set(_newImage, _alignObj)
 */
export function assignValueAtKey(parent, targetKey, target) {
	if (!parent) throw new Error("ObjectUtils.assignValueAtKey() - 'parent' parameter must be defined, isn't")
	for (var subKey in target) {
		// reassign the target key from a string to a child of the parent object if a parent is defined
		if (subKey === targetKey && typeof target[subKey] === 'string') {
			if (parent[target[subKey]]) target[subKey] = parent[target[subKey]]
			else throw new Error(`parent[${target[subKey]}] is undefined`)
		}
		// if the value at the subKey is also an object, continue digging
		if (target[subKey] && target[subKey].toString() === '[object Object]')
			assignValueAtKey(parent, targetKey, target[subKey])
	}
}

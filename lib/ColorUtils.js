...
export function tint(obj) {
	...
	for (var val in obj.from)
		returnColors[val] =
			val === 'a'
				? parseInt(obj.from[val])
				: Math.round(parseInt(obj.from[val]) + (parseInt(obj.to[val]) - parseInt(obj.from[val])) * obj.amount)
...
export function transform(obj) {
	...
	for (var val in obj.from)
		returnColors[val] =
			val === 'a' ? parseInt(obj.from[val]) : Math.round(parseInt(obj.from[val]) - (255 - parseInt(obj.to[val])) * obj.amount)
